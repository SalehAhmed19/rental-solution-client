...
import L from "leaflet";
import debounce from "lodash.debounce";
import { usePostAdMutation } from "../../redux/slices/propertyApi"; // আপনার বানানো হুক
import { toast } from "sonner"; // বা আপনার পছন্দের নোটিফিকেশন লাইব্রেরি
...
export default function AdPost() {
  ...
  const [postAd, { isLoading }] = usePostAdMutation(); // RTK Mutation
  ...
  const searchLocation = async (query: string) => {
    if (!query || query.length < 4) return;
    const searchQuery = query.split(",").slice(-2).join(", ");
    try {
      const response = await fetch(
        `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(searchQuery + ", Dhaka, Bangladesh")}&limit=1`,
      );
  ...
  const handleChange = (
    ...
    if (name === "area" || (name === "fullAddress" && value.length > 10)) {
      debouncedSearch(value);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    // টেক্সট ডাটা অ্যাপেন্ড করা (মডেলের স্ট্রাকচার অনুযায়ী)
    ...
    // বুলিয়ান ফিচারস
    data.append("hasLift", String(formData.hasLift));
    data.append("hasGenerator", String(formData.hasGenerator));
    data.append("hasParking", String(formData.hasParking));
    data.append("hasGasConnection", String(formData.hasGasConnection));

    // GeoJSON Coordinates: [longitude, latitude] - মঙ্গুস অনুযায়ী
    const coords = [formData.coordinates.lng, formData.coordinates.lat];
    data.append("coordinates", JSON.stringify(coords));

    // ইমেজ ফাইলগুলো অ্যাপেন্ড করা
          ...
          <form className="space-y-10" onSubmit={(e) => e.preventDefault()}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4">
                  {[
                    ...
                    >
                      ...
                      />
                      <span className="text-sm font-bold text-gray-600 group-hover:text-teal-600 transition-colors">
                        {item.label}
                      </span>
                    </label>
                  ))}
            ...
            <div className="flex justify-between items-center pt-8 border-t border-gray-50">
              ...
              <Button
                ...
                disabled={isLoading}
                className={`h-16 px-12 rounded-2xl font-black text-white shadow-xl transition-all active:scale-95 ${step === totalSteps ? "bg-teal-600 hover:bg-teal-700" : "bg-[#051c1e] hover:bg-black"}`}
              >
                {isLoading
                  ? "প্রসেস হচ্ছে..."
                  : step === totalSteps
