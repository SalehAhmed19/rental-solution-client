...
import { useNavigate } from "react-router-dom";
import { MapPin, ArrowRight, Compass, Building2, Loader2 } from "lucide-react";
import {
  Carousel,
  CarouselContent,
  CarouselItem,
...
// ১. এপিআই হুক ইমপোর্ট (আপনার প্রজেক্টের পাথ অনুযায়ী)
import { useGetPropertiesQuery } from "@/redux/slices/propertyApi";

// লোকেশন অনুযায়ী ডামি ইমেজ ম্যাপ (আগের মতোই থাকবে)
const locationImages: Record<string, { img: string; tag: string }> = {
  Uttara: {
    img: "https://images.unsplash.com/photo-1512917774080-9991f1c4c750?q=80&w=800",
    tag: "শান্ত এলাকা",
...
export default function LocationCarousel() {
  ...
  const { data: response, isLoading } = useGetPropertiesQuery(undefined);
  const allProperties = response?.data || [];

  // ৩. ডাইনামিক লোকেশন ক্যালকুলেশন (ডাটাবেজ ডাটা থেকে)
  const dynamicLocations = Object.entries(
    allProperties.reduce((acc: Record<string, number>, curr: any) => {
      // আপনার স্কিমা অনুযায়ী address.area ধরছি
      const area = curr.address?.area;
      if (area) {
        acc[area] = (acc[area] || 0) + 1;
      }
